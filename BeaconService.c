...
//#define TIMER_9_TICKS 10 // before it was 70 , overdefined in beconservice.h also
...
ES_Event RunBeaconService(ES_Event ThisEvent) {
     ...
     *******************************************/
    int currentBeacon = CheckBeacon(); // read all bumpers
    static int lastBeacon = 0;
    static unsigned char beaconMem = 0x0;
    ES_EventTyp_t curEvent = ES_NO_EVENT;
    static ES_EventTyp_t lastEvent = ES_NO_EVENT;
   // printf("CheckBeacon= %x /n", currentBeacon  );
    ...
    switch (ThisEvent.EventType) {
        ...
        case ES_TIMEOUT:
            ES_Timer_InitTimer(BEACON_SERVICE_TIMER, TIMER_9_TICKS);
            beaconMem <<= 1;
            //printf("BeaconMem = %x /n", beaconMem);
