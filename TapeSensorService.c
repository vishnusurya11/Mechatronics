...
ES_Event RunTapeSensorService(ES_Event ThisEvent) {
     ...
     *******************************************/
    static unsigned char curTapeS = 0;
    static unsigned char lastTapeS = 0;
    ...
    switch (ThisEvent.EventType) {
        ...
        case ES_TIMEOUT:
            ...
            //reset the tape values if ready, otherwise keep stepping through the switch statements
            if (timerCount == 3) {

                curTapeS = CheckTapeSensors(timerCount);
               // printf("\n curtape = %d\n", curTapeS);
                //printf("\n lasttape = %d\n", lastTapeS);
                if ((curTapeS != lastTapeS)) {
                    // printf("\n *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-tape event*-*-*-*-*-*-*-*-*-%x*-*-*-*-*-*-*-*-*- \n",curTapeS);
                    //printf("\n curtape = %d\n", curTapeS);
                    // printf("\n lasttape = %d\n", lastTapeS);
                    lastTapeS = curTapeS;
                    ReturnEvent.EventType = TAPE_EVENT;
                    ReturnEvent.EventParam = curTapeS;
#ifndef SIMPLESERVICE_TEST           // keep this as is for test harness
                    PostHSM(ReturnEvent);
#else
                    PostTapeSensorService(ReturnEvent);
#endif  
                    // printf("\n we made it. here's the event:%c \n", curTapeS);
                } else {
                    ReturnEvent.EventType = ES_NO_EVENT;
                }
            } else {
                CheckTapeSensors(timerCount);
                ReturnEvent.EventType = ES_NO_EVENT;
            }
