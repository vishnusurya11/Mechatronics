...
static BounceOffTapeSubHSMState_t CurrentState = Turning; // <- change name to match ENUM
...
/**
 * @Function InitBounceOffTapeSubHSM(uint8_t Priority)
...
uint8_t InitBounceOffTapeSubHSM(void) {
    ES_Event returnEvent;

    CurrentState = Turning;
    returnEvent = RunBounceOffTapeSubHSM(INIT_EVENT);
...
/**
 * @Function RunBounceOffTapeSubHSM(ES_Event ThisEvent)
...
ES_Event RunBounceOffTapeSubHSM(ES_Event ThisEvent) {
    uint8_t makeTransition = FALSE; // use to flag transition
    BounceOffTapeSubHSMState_t nextState; // <- change type to correct enum
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunBounceOffTapeSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        //printf("\n nextstate=%d \n", CurrentState);
        RunBounceOffTapeSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
